...
PARENT_PPATH = SCRIPT_PATH
LOKI_PATH = PARENT_PPATH /"LOKI"
LOG_FILE = SCRIPT_PATH / "logs" / "logs.log"
THRESHOLD_MULTIPLIER = 0.5

# Output directories
AVATAR_OUTPUT_DIR = PARENT_PPATH / "loki_training_data" / "pedestrian_avatars"
FEATURES_OUTPUT_DIR = PARENT_PPATH / "loki_training_data" / "pedestrian_features"
...
def run_pedestrian_map_aligner():
    ...
    map_aligner = PedestrianMapAligner(
        scenario_path=None,
        loki_csv_path=LOKI_PATH / "b_avatar_filtered_pedestrians.csv",
        data_path=LOKI_PATH,
    )
    map_aligner.align(
        save=True,
...
def main():
    try:
        ...
        df_pedestrians_filtered.to_csv(LOKI_PATH / "avatar_filtered_pedestrians.csv", index=False)
        logger.info(
        ...
        df_binarized = load_and_binarize_intention(LOKI_PATH / "avatar_filtered_pedestrians.csv")
        df_binarized.to_csv(LOKI_PATH / "b_avatar_filtered_pedestrians.csv", index=False)
